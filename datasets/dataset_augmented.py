...
# from https://github.com/facebookresearch/WavAugment
import augment
...
class LADataset(Dataset):
    '''
        ...
        Input :
            ...
            data_augment : the probability of how to augment the input data
    ...
    def __init__(self, split='train', txtpath=None, datadir=None, transforms=None,
                 f_len=750, padding='repeat', num_features=20, n_fft=512, w_len=0.02, norm=None, expand=False, speaker=False, data_augment=None):
    ...
    def get_audio(self, idx):
        ...
        return audio, rate, label, sample_path
    ...
    def __getitem__(self, idx):
        ...
        # now augment the data - what should we use?
        # => use WavAugment!
        datachain = augment.EffectChain()
        if self.data_augment is not None:
            #  random pitch
            if np.random.uniform(0, 1) < self.data_augment:
                datachain = datachain.pitch(np.random.randint(-200, 200))
            # audio reverberation
            if np.random.uniform(0, 1) < self.data_augment:
                datachain = datachain.reverb(np.random.randint(0, 100), np.random.randint(0, 100), np.random.randint(0, 100)).channels(1)
            # random noise
            if np.random.uniform(0, 1) < self.data_augment:
                datachain = datachain.additive_noise(lambda: torch.zeros_like(audio_tensor).uniform_(), snr=np.random.uniform(10, 20))
            # time dropout
            if np.random.uniform(0, 1) < self.data_augment:
                datachain = datachain.time_dropout(max_frames=len(audio_tensor) / np.random.uniform(2, 10))
            # bandreject
            if np.random.uniform(0, 1) < self.data_augment:
                datachain = datachain.sinc('-a', '120', '500-100')
            if np.random.uniform(0, 1) < self.data_augment:
                datachain = datachain.clip(np.random.triangular(0.1, 1.0, 1.0))
            audio = datachain.apply(audio_tensor, src_info={'rate' : rate}).numpy()            
